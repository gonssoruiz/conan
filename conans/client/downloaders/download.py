...
def run_downloader(requester, output, verify, config, user_download=False, use_cache=True, use_remote_cache=False, **kwargs):
    ...
    if use_cache and config.download_cache:
        if use_remote_cache and config.remote_cache:
